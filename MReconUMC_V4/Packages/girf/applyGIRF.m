...
F_wf=fftshift(fft(fftshift(wf0,1),[],1),1);
...
if strcmpi(preemp,'no')
    F_wf_corr=F_wf.*rs_girf;
else
    %%% Preemp mode %%%
    nf=numel(f_wf);
    pe_filter=zeros(size(rs_girf(:,1)));
    pe_filter(nf/2+1-round(preemp_bw/2/df_wf):nf/2+1+round(preemp_bw/2/df_wf))=1;
...
if strcmpi(vis,'yes');
    ...
    uwangle1=unwrap(angle(rs_girf(:,1)));cp=floor(size(uwangle1,1)/2)+1;legend('M')
