...
if strcmpi(MR.Parameter.Scan.AcqMode,'Radial') && strcmpi(MR.Parameter.Scan.UTE,'no')
    ...
    if strcmpi(MR.UMCParameters.AdjointReconstruction.NUFFTMethod,'mrecon')  % Conventional gridder
        ...
        if MR.UMCParameters.AdjointReconstruction.Goldenangle>0
         ...
         if strcmpi(MR.UMCParameters.AdjointReconstruction.NUFFTMethod,'greengard');MR.Parameter.Gridder.RadialAngles=cellfun(@(x) x-pi/2,MR.Parameter.Gridder.RadialAngles,'UniformOutput',false);end
         if strcmpi(MR.UMCParameters.AdjointReconstruction.NUFFTMethod,'fessler');MR.Parameter.Gridder.RadialAngles=cellfun(@(x) x-3*pi/2,MR.Parameter.Gridder.RadialAngles,'UniformOutput',false);end
         ...
         MR.Parameter.Gridder.Weights=radial_dcf(MR.UMCParameters.AdjointReconstruction.KspaceSize);
...
elseif strcmpi(MR.Parameter.Scan.UTE,'yes')
    ...
    % THe reconframe function converts cell2mat, reverse the operation
    %MR.GridderCalculateTrajectory;
    MR.Parameter.Gridder.RadialAngles={MR.Parameter.Gridder.RadialAngles};
    %MR.Data={MR.Data};
    %MR.Parameter.Gridder.Kpos={};
    %MR.Parameter.Gridder.Weights={};
