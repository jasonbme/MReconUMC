...
Nd = size(b1(:,:,1)); % Image dimensions
Jd = [6,6]; % Neighbours used in each direction
Kd = floor([Nd*1.5]); % FFT sizes for radial
n_shift = Nd/2 ;
for tt=1:size(k,4), % Loop over all time frames
	kk=k(:,:,:,tt);
	om = [real(kk(:)), imag(kk(:))]*2*pi; % Make digital frequencies in radians
