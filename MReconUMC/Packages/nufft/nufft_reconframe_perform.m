...
%MR.Parameter.Gridder.OutputMatrixSize=[round(MR.Parameter.Scan.Samples(1))...
%    round(MR.Parameter.Scan.Samples(1)) round(MR.Parameter.Scan.Samples(3)*MR.Parameter.Encoding.KzOversampling)];
...
data=MR.Data;
kpos=cellfun(@(x) permute(x*2*MR.Parameter.Scan.Samples(1),[2 3 4 1 5 6 7 8 9 10 11 12 13]),MR.Parameter.Gridder.Kpos,'UniformOutput',false);
...
for n=1:num_data            
    ...
    for dyn=1:MR.UMCParameters.AdjointReconstruction.IspaceSize{n}(5)
        ...
        MR.Parameter.Gridder.RadialAngles=permute(squeeze(angles{n}(:,:,:,:,dyn)),[2 1]);
