...
if strcmpi(MR.UMCParameters.LinearReconstruction.Autocalibrate,'yes')
    ...
    if strcmpi(MR.UMCParameters.LinearReconstruction.AutocalibrateLoad,'yes') && exist('csmac.mat')==2
        % Notification
...
%% Reference Scan
if strcmpi(MR.UMCParameters.LinearReconstruction.CoilReferenceScan,'yes')
    
     % Check whether coils maps are available in the folder
     cd(MR.UMCParameters.GeneralComputing.TemporateWorkingDirectory)
     
     if strcmpi(MR.UMCParameters.LinearReconstruction.CoilReferenceScanLoad,'yes') && exist('csmref.mat')==2
         
        % Notification
        fprintf('Load coil maps from directory (MRsense)...........  ');tic;
        load('csmref.mat')
        MR.Parameter.Recon.Sensitivities=csmref;
        MR.UMCParameters.LinearReconstruction.CombineCoilsOperator=CC(MR.Parameter.Recon.Sensitivities);
        cd(MR.UMCParameters.GeneralComputing.PermanentWorkingDirectory)
