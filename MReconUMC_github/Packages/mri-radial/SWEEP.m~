...
% Select middle slice slice if 3D
...
% Compute L2 per coil
L2=squeeze(sqrt(squeeze(sum(sum(abs(MR.Data).^2,1),2)))); 
...
model=@(a,theta)(cos(theta).^2*a(1)+sin(theta).^2*a(2));
A0=[1 1];
A=nlinfit(MR.Parameter.Gridder.RadialAngles,cshift,model,A0);

% Reshape MR.Data to for easier handling --> [nx ns*ndyn nz ncoils 1]
MR.Data=permute(reshape(permute(MR.Data,[1 3 4 2 5]),[nx nz ncoils ns*ndyn 1]),[1 4 2 3 5]);
MR.Data=permute(reshape(MR.Data,[nx ns ndyn 1 ncoils]),[1 2 4 5 3]);
