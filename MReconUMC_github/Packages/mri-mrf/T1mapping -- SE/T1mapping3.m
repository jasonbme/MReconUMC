function [T1map,IM,Amap,Cmap]=T1mapping3(IM,ITlist)
%% Methodology
% This function does simple 3 parameter T1 mapping for IR-SE sequences.
% M = C + A * exp(-t/T1)
...
Cmap=zeros(numel(IM(:,:,1)),1);   % Allocate memory for Cmap.
...
% Fitting settings
F = @(x,xdata)x(1)+x(2)*exp(-xdata/x(3)); % Model
x0 = [-1 2 1000]; % Initial guess
...
for j=1:numel(IM(:,:,1))
    ...
    if (reshape_sum_IM(j)<0.05*avgmax)  % If total pixel intensity is to low, its probably background.
        ...
        if (fit(3)>3000)
            T1map(j)=3000;     % Simply to increase the contrast in for lower T1s.
        else
            T1map(j)=fit(3);
            Cmap(j)=fit(1);
            Amap(j)=fit(2);
...
Cmap=reshape(Cmap,[size(IM(:,:,1),1),size(IM(:,:,1),2)]);
