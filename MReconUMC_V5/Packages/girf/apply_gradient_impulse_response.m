...
if MR.UMCParameters.ReconFlags.Verbose
    ...
    hold on;for n=1:numel(t_adc);for k=1:size(t_adc{n},2);scatter(t_adc{n}(1:incr:end,k),interp1(t,wf(:,1),t_adc{n}(1:incr:end,k)),5,'r');end;end;set(gca,'LineWidth',2,'FontSize',12,'FontWeight','bold')
    ...
    hold on;for n=1:numel(t_adc);for k=1:size(t_adc{n},2);scatter(t_adc{n}(1:incr:end,k),interp1(t,MR.UMCParameters.SystemCorrections.GIRF_output_waveforms(:,1),t_adc{n}(1:incr:end,k)),5,'r');hold on;end;end;legend('M','P','S');set(gca,'LineWidth',2,'FontSize',12,'FontWeight','bold')
