...
for z=1:Kd{n}(3)    % Z
    for nl=1:Kd{n}(2)
        Kpos{n}(:,:,nl,z,1,dyn,ph,ech,loc,mix,ex1,ex2)=...
            [interp1qr(MR.UMCParameters.SystemCorrections.GIRF_time,k_real(MR.Parameter.Gridder.RadialAngles{n}(:,nl,z,1,dyn,ph,ech,loc,mix,ex1,ex2),k_accumulated(:,1)),MR.UMCParameters.SystemCorrections.GIRF_ADC_time{n}(:,ech)) ...
            interp1qr(MR.UMCParameters.SystemCorrections.GIRF_time,k_imag(MR.Parameter.Gridder.RadialAngles{n}(:,nl,z,1,dyn,ph,ech,loc,mix,ex1,ex2),k_accumulated(:,2)),MR.UMCParameters.SystemCorrections.GIRF_ADC_time{n}(:,ech)) ...
            zeros(numel(MR.UMCParameters.SystemCorrections.GIRF_ADC_time{n}(:,ech)),1)];
        
        Kpos_nom{n}(:,:,nl,z,1,dyn,ph,ech,loc,mix,ex1,ex2)=...
            [interp1qr(MR.UMCParameters.SystemCorrections.GIRF_time,k_real(MR.Parameter.Gridder.RadialAngles{n}(:,nl,z,1,dyn,ph,ech,loc,mix,ex1,ex2),k_accumulated_nom(:,1)),MR.UMCParameters.SystemCorrections.GIRF_ADC_time{n}(:,ech)) ...
            interp1qr(MR.UMCParameters.SystemCorrections.GIRF_time,k_imag(MR.Parameter.Gridder.RadialAngles{n}(:,nl,z,1,dyn,ph,ech,loc,mix,ex1,ex2),k_accumulated_nom(:,2)),MR.UMCParameters.SystemCorrections.GIRF_ADC_time{n}(:,ech)) ...
            zeros(numel(MR.UMCParameters.SystemCorrections.GIRF_ADC_time{n}(:,ech)),1)];
    end
end % Z
end % Dynamics
...
if MR.UMCParameters.ReconFlags.Verbose
    subplot(337);for n=1:num_data;for ech=1:Kd{n}(7);
            ...
            plot(MR.UMCParameters.SystemCorrections.GIRF_ADC_time{n}(:,ech),sqrt(M1.*conj(M1)+M2.*conj(M2)),'k','Linewidth',2);hold on;
            plot(MR.UMCParameters.SystemCorrections.GIRF_ADC_time{n}(:,ech),sqrt(N1.*conj(N1)+N2.*conj(N2)),'r--','Linewidth',2);hold on;end;end;grid on;box on;

            axis([MR.UMCParameters.SystemCorrections.GIRF_ADC_time{1}(1,1) MR.UMCParameters.SystemCorrections.GIRF_ADC_time{num_data}(end,Kd{num_data}(7)) 0 0.75 ]);
