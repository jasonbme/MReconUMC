...
for n=1:num_data;
	...
	if MR.UMCParameters.AdjointReconstruction.Goldenangle>0
        d_angle=-1*(pi/(((1+sqrt(5))/2)+MR.UMCParameters.AdjointReconstruction.Goldenangle-1));
    ...
    MR.Parameter.Gridder.RadialAngles{n}=mod(MR.Parameter.Gridder.RadialAngles{n}+pi/2,-2*pi);
end
