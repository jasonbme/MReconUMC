...
switch MR.UMCParameters.AdjointReconstruction.CoilSensitivityMaps
    ...
    case 'walsh'
        % Notification
        fprintf('Estimate coil maps (Walsh)........................  ');tic; 
        ...
        % Some exceptions for the 'mrecon' gridder
        if ~strcmpi(MR.UMCParameters.AdjointReconstruction.NUFFTMethod,'mrecon')
            MR.Parameter.Gridder.Weights={ipermute(reshape(permute(MR.Parameter.Gridder.Weights{necho},[1 2 3 5 4 6 7 8 9 10 11 12]),...
                [dims{necho}(1) dims{necho}(2)*dims{necho}(5) size(MR.Parameter.Gridder.Weights{necho},3)*dims{necho}(5) dims{necho}(6:end) ]),[1 2 3 5 4 6 7 8 9 10 11 12])};
            MR.Parameter.Gridder.Kpos={ipermute(reshape(permute(MR.Parameter.Gridder.Kpos{necho},[1 2 3 4 6 5 7 8 9 10 11 12 13]),...
                [3 dims{necho}(1) dims{necho}(2)*dims{necho}(5) size(MR.Parameter.Gridder.Kpos{necho},4)*dims{necho}(5) dims{necho}(6:end)]),[1 2 3 4 6 5 7 8 9 10 11 12 13])};end
        if strcmpi(MR.UMCParameters.AdjointReconstruction.NUFFTMethod,'mrecon');MR.Parameter.Scan.Samples=[dims{necho}(1) dims{necho}(2) dims{necho}(3)];end    

        % Do the NUFFT        
        % Notification
        fprintf('\n     Perform the nufft on all the data............  ');tic; 
        MR.UMCParameters.ReconFlags.nufft_csmapping=1; % Flag is for fprintf notifications
        AdjointReconstruction(MR); 
        MR.UMCParameters.ReconFlags.nufft_csmapping=0;
